...
blk = b.genesis({v: 10 ** 18})
tx4 = t.contract(0, 0, 10 ** 12, 1000, code2).sign(k)
...
tx10 = t.contract(0, 0, 10 ** 12, 1000, code3).sign(k)
...
tx12 = t.Transaction(
    2, 0, 10 ** 12, 1000, addr, serpent.encode_datalist([500])).sign(k)
o = pb.apply_tx(blk, tx12)
print "Balance", serpent.decode_datalist(o)
tx13 = t.Transaction(
    3, 0, 10 ** 12, 1000, addr, serpent.encode_datalist([500, 726])).sign(k)
o = pb.apply_tx(blk, tx13)
print "Set balance to 500:726", serpent.decode_datalist(o)
tx14 = t.Transaction(
    4, 0, 10 ** 12, 1000, addr, serpent.encode_datalist([500])).sign(k)
...
tx15 = t.contract(5, 0, 10 ** 12, 2000, code4).sign(k)
addr2 = pb.apply_tx(blk, tx15)
print "Address:", addr.encode('hex')
tx16 = t.Transaction(
    6, 10 ** 17, 10 ** 12, 2000, addr2, serpent.encode_datalist([500])).sign(k)
o = pb.apply_tx(blk, tx16)
print "First participant added", serpent.decode_datalist(o)
tx17 = t.Transaction(
    0, 10 ** 17, 10 ** 12, 2000, addr2, serpent.encode_datalist([500])).sign(k2)
...
tx19 = t.Transaction(
    8, 0, 10 ** 12, 2000, addr, serpent.encode_datalist([500, 300])).sign(k)
